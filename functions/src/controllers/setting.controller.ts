...
import { fieldsSchema, postSchema, patchSchema } from '../models/setting.models';
import { Request, Response } from "express";

const settings = new GlobalReq('settings', fieldsSchema);

export const singleGet = async (req: Request, res: Response) => {
  try {
    if (!req.params.id) throw new Error('id is required');

    let doc = await settings.getSingleDoc(req.params.id);
...
export const countMethod = async (req: Request, res: Response) => {
  try {
    ...
    const size = await settings.countDocuments(filter);
...
export const getMethod = async (req: Request, res: Response) => {
  try {
    ...
    const arrRes = await settings.getCollection(filter);
...
export const postMethod = async (req: Request, res: Response) => {
  try {
    ...
    let obj = await settings.addDocument({ ...value, createdAt: Date.now() });
    return res.json(obj);
  } catch (err: Any) {
    console.log(err);
...
export const patchMethod = async (req: Request, res: Response) => {
  try {
    ...
    let obj = await settings.setDocument(req.params.id, value);
    return res.json(obj);
  } catch (err: Any) {
    console.log(err);
    let msg = err.details ? err.details : err.msg
...
export const deleteMethod = async (req: Request, res: Response) => {
  try {
    ...
    await settings.deleteDocument(req.params.id);
